...
VirtualArcItem::VirtualArcItem(const QLineF& firstLine, const QLineF& secondLine, QGraphicsItem* parent)
	: TrackLineItem(firstLine, parent), secondLine(secondLine) {
	createSelectionPolygon();
}

VirtualArcItem::~VirtualArcItem() {
}

QRectF VirtualArcItem::boundingRect() const {
	return selectionPolygon.boundingRect();
}

QPainterPath VirtualArcItem::shape() const {
	QPainterPath ret;
	ret.addPolygon(selectionPolygon);
	return ret;
}

void VirtualArcItem::createSelectionPolygon() {
	QPolygonF nPolygon;
...
void VirtualArcItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
	Q_UNUSED(option);
	Q_UNUSED(widget);
