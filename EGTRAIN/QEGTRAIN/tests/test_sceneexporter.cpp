...
int main() {
	...
	// 17. Incidents export to the legacy Incidents.txt
	{
		...
		std::ofstream(scene / "infrastructure.json")
			...
			<< "\n";
		std::ofstream(scene / "stations.json") << R"({"stations":[{"id":"st","name":"Station","platforms":[]}]})" << "\n";
		std::ofstream(scene / "signalling.json") << R"({"signals":[{"id":"sig-x","protected_section":"@canonical-block@"},{"id":"sig-switch","protected_section":"@canonical-block@--1.000000/@canonical-other@-0.000000"},{"id":"sig-malformed","protected_section":"0-B0/1-B0"}],"routes":[{"id":"route0","blocks":["canonical-block","@canonical-block@--1.000000/@canonical-other@-0.000000"]}]})" << "\n";
		std::ofstream(scene / "rolling_stock.json")
