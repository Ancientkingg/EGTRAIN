...
| Other GUI coordinate/display files | derived/output | The native GUI derives fallback layout from canonical node and station coordinates when the two explicit `views.json` sources are absent. Historical hidden-track, virtual-link, and HTML-template files are not a second infrastructure model and are no longer normal runtime input. |
